...
export default function Detections() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => api.getDetections({ page, severity: severityFilter }),
  })
  ...
  return (
    <div>
      ...
      <div className="mt-4 rounded-xl border border-gray-200 bg-white shadow-sm">
              ...
              {!isLoading && detections.map((det) => (
                ...
                >
                  <td className="px-6 py-3">
                    ...
                    {det.text && (
                      <div className="mt-0.5 text-xs text-gray-500 line-clamp-1">{det.text}</div>
                    )}
                  </td>
        ...
        {total > perPage && (
          <div className="flex items-center justify-between border-t border-gray-100 px-6 py-3">
            <span className="text-sm text-gray-500">Page {page} of {Math.ceil(total / perPage)}</span>
      ...
      {selectedDet && (
        <div className="fixed inset-0 z-50 flex justify-end bg-black/20" onClick={() => setSelectedDet(null)}>
          <div
            className="h-full w-full max-w-2xl overflow-y-auto bg-white shadow-xl"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="sticky top-0 z-10 flex items-center justify-between border-b border-gray-200 bg-white px-6 py-4">
              <h2 className="text-lg font-semibold text-gray-900">Detection Detail</h2>
              <button
            ...
            <div className="space-y-6 p-6">
              <div>
                ...
                <div className="mt-2 flex items-center gap-3">
                  ...
                  <span className="text-sm text-gray-500">{new Date(selectedDet.timestamp).toLocaleString()}</span>
                </div>
              </div>
