...
function ProcessNode({ data }: { data: Record<string, unknown> }) {
  ...
  const bg = d.isFocus ? 'bg-red-50' : d.isOnFocusPath ? 'bg-blue-50/60' : 'bg-white'
  ...
  return (
    <div className={`rounded-lg border-2 px-3 py-2 shadow-sm ${bg} ${d.selected ? 'ring-2 ring-fibratus-200' : ''}`}
      ...
      <div className="flex items-center gap-2">
        ...
        <span className={'font-medium text-sm truncate flex-1 ' + (d.isFocus ? 'text-red-700' : 'text-gray-900')}>{d.name}</span>
        {d.isFocus && <span className="rounded bg-red-100 text-red-600 text-[9px] px-1 py-0.5 font-bold flex-shrink-0">TRIGGER</span>}
      </div>
      {d.exe && <div className="mt-1 text-[10px] text-gray-400 font-mono truncate">{d.exe}</div>}
      ...
      <div className="mt-1.5 flex items-center gap-2">
        ...
        {d.hasChildren && (
          <button className="flex items-center gap-1 text-[10px] text-gray-500 hover:text-gray-700 ml-auto"
...
function EventGroupNode({ data }: { data: Record<string, unknown> }) {
  ...
  return (
    <div className={'rounded-md border px-2.5 py-1.5 shadow-sm bg-white ' + (d.selected ? 'ring-2 ring-fibratus-200' : '')}
      ...
      <div className="flex items-center gap-2">
        ...
        <span className="text-[10px] text-gray-500 font-medium">{d.count} event{d.count > 1 ? 's' : ''}</span>
      </div>
      {d.topItems.length > 0 && (
        <div className="mt-1 space-y-0.5">
          {d.topItems.map((item, i) => (
            <div key={i} className="text-[10px] text-gray-600 font-mono truncate">{item}</div>
          ))}
          {d.count > d.topItems.length && (
            <div className="text-[9px] text-gray-400">+{d.count - d.topItems.length} more</div>
          )}
        </div>
      )}
    </div>
...
export default function DetectionProcessGraph({ detection, focusPid }: Props) {
  ...
  if (isLoading) return <div className="flex items-center justify-center h-64 text-gray-400">Loading process tree...</div>
  if (byPid.size === 0) return <div className="flex items-center justify-center h-64 text-sm text-gray-400">No process events found.</div>
    ...
    <div className="flex flex-col" style={{ height: 'calc(100vh - 200px)' }}>
      ...
      <div className="flex items-center gap-3 px-4 py-2.5 border-b bg-gray-50/50 flex-shrink-0">
        ...
        {detection.labels?.['technique.id'] && (
          <span className="rounded bg-gray-100 px-1.5 py-0.5 text-xs font-mono text-gray-600">{detection.labels['technique.id']}</span>
        )}
        ...
        <div className="w-80 border-l overflow-auto bg-white flex-shrink-0">
          {selectedProcess ? (
            <div className="flex flex-col h-full">
              <div className="p-4 border-b space-y-2 flex-shrink-0">
                ...
                {selectedProcess.parentPid > 0 && (
                  byPid.has(selectedProcess.parentPid) ? (
                    <p className="text-xs text-gray-500">Parent: <button className="text-fibratus-600 hover:underline"
                ...
                {selectedProcess.childPids.length > 0 ? (
                  <div className="flex flex-wrap gap-1">
                    ...
                    {selectedProcess.childPids.map(cpid => {
                      ...
                      return child ? (
                        <button key={cpid} className="rounded bg-gray-100 px-1.5 py-0.5 text-[10px] text-gray-700 hover:bg-gray-200"
              ...
              {/* Event tabs */}
              <div className="px-3 py-2 border-b flex items-center gap-1 flex-shrink-0 flex-wrap">
                <button onClick={() => setDetailTab('all')}
                  className={'rounded-full px-2 py-0.5 text-[10px] font-medium border ' +
                    (detailTab === 'all' ? 'bg-gray-800 text-white border-gray-800' : 'bg-white text-gray-500 border-gray-200')}>
                ...
                {Object.entries(selectedProcess.eventsByCategory).map(([cat, count]) => (
                  <button key={cat} onClick={() => setDetailTab(detailTab === cat ? 'all' : cat)}
                    className={'rounded-full px-2 py-0.5 text-[10px] font-medium border ' +
                      (detailTab === cat ? 'bg-gray-800 text-white border-gray-800' : 'bg-white text-gray-500 border-gray-200')}>
                    {catLabel(cat)} ({count})
                  </button>
                ))}
              </div>
              <div className="flex-1 overflow-auto">
                ...
                {detailEvents.length === 0 && <div className="text-xs text-gray-400 text-center py-8">No events</div>}
              </div>
            </div>
          ) : selectedEventGroup ? (
            <div className="flex flex-col h-full">
              <div className="p-4 border-b flex-shrink-0">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-gray-500">{selectedEventGroup.events.length} events</span>
                </div>
            ...
            <div className="p-4 text-sm text-gray-400 text-center mt-20">Click a node to view details</div>
          )}
        </div>
      </div>
    </div>
