    ...
    <div className="flex flex-col gap-3 h-full">
      <form onSubmit={e => { e.preventDefault(); browse(pathInput.trim() || 'C:\\') }} className="flex gap-2">
        <input value={pathInput} onChange={e => setPathInput(e.target.value)}
          className="flex-1 rounded border px-3 py-2 text-sm font-mono focus:ring-2 focus:ring-fibratus-500 focus:outline-none" placeholder="Path..." />
        <button type="button" onClick={() => browse(path)} disabled={loading} className="p-2 text-gray-500 hover:text-fibratus-600 disabled:opacity-50">
          <RefreshCw className="w-4 h-4" />
        </button>
      </form>

      <div className="flex items-center gap-1 text-sm flex-wrap bg-gray-50 rounded px-3 py-1.5">
        {segments.map((s, i) => (
          <span key={i} className="flex items-center gap-1">
            {i > 0 && <ChevronRight className="w-3 h-3 text-gray-400" />}
            <button onClick={() => browse(segments.slice(0,i+1).join('\\') + (i===0?'\\':''))} className="text-fibratus-600 hover:underline">{s}</button>
          </span>
        ))}
      </div>
          ...
          <table className="w-full text-sm">
            <thead className="bg-gray-50 sticky top-0">
              <tr>
                ...
                <th className="text-right px-3 py-2 w-44">Modified</th>
              </tr>
            </thead>
            <tbody>
              {segments.length > 1 && (
                <tr className="hover:bg-gray-50 cursor-pointer" onClick={goUp}>
                  <td className="px-3 py-1.5"><ArrowUp className="w-4 h-4 text-gray-400" /></td>
                  <td className="px-3 py-1.5 text-gray-500">..</td><td /><td />
                </tr>
              )}
              {entries.map(e => (
                <tr key={e.name} className="hover:bg-gray-50 cursor-pointer border-t border-gray-100" onClick={() => nav(e)}>
                  <td className="px-3 py-1.5">{e.is_dir ? <Folder className="w-4 h-4 text-amber-500" /> : <File className="w-4 h-4 text-gray-400" />}</td>
                  <td className="px-3 py-1.5 font-mono">{e.name}</td>
                  <td className="px-3 py-1.5 text-right text-gray-500 tabular-nums">{e.is_dir ? '-' : fmtSize(e.size)}</td>
                  <td className="px-3 py-1.5 text-right text-gray-500 text-xs">{fmtDate(e.mod_time)}</td>
                </tr>
              ))}
              {entries.length === 0 && <tr><td colSpan={4} className="text-center py-8 text-gray-400">Empty</td></tr>}
            </tbody>
          </table>
        </div>
      )}

      {previewLoading && <div className="flex items-center gap-2 text-sm text-gray-500"><Loader2 className="w-4 h-4 animate-spin" />Fetching file...</div>}
      {preview && (
        <div className="border rounded p-3 bg-gray-50 mt-2">
          <div className="flex items-center justify-between mb-2">
            <span className="font-mono text-xs text-gray-600">{preview.path}</span>
            <div className="flex items-center gap-3 text-xs text-gray-500">
              ...
              <button onClick={() => setPreview(null)} className="text-gray-400 hover:text-gray-600">x</button>
            </div>
          </div>
          {preview.size < 262144 ? (
            <pre className="text-xs font-mono bg-gray-900 text-gray-100 rounded p-3 max-h-64 overflow-auto whitespace-pre-wrap break-all">
              {(() => { try { return atob(preview.content) } catch { return '[binary]' } })()}
            </pre>
          ) : <p className="text-sm text-gray-500 text-center py-4">File too large to preview. Download instead.</p>}
        </div>
      )}
    </div>
  )
