...
// Macro represents a filter macro that can expand to an
...
// MacroConfig provides macro lookup for the parser.
type MacroConfig struct {
	macros map[string]*Macro
}

// NewMacroConfig creates a new macro config from a map of macros.
func NewMacroConfig(macros map[string]*Macro) *MacroConfig {
	return &MacroConfig{macros: macros}
}

// GetMacro returns the macro for the given identifier.
func (c *MacroConfig) GetMacro(id string) *Macro {
	if c == nil {
		return nil
	}
	return c.macros[id]
...
// IsMacroList returns true if the given identifier is a macro that expands to a list.
func (c *MacroConfig) IsMacroList(id string) bool {
	if c == nil {
		return false
	}
	macro, ok := c.macros[id]
	if !ok {
		return false
	}
...
// NewParserWithConfig builds a new parser instance with macro config.
func NewParserWithConfig(expr string, config *MacroConfig) *Parser {
...
func (p *Parser) parseUnaryExpr() (Expr, error) {
	...
	case Ident:
		...
		if p.c != nil {
			...
			if macro != nil {
				if macro.Expr != "" {
					mp := NewParserWithConfig(macro.Expr, p.c)
					expr, err := mp.ParseExpr()
					if err != nil {
						return nil, fmt.Errorf("syntax error in %q macro: %w", lit, err)
					}
					return expr, nil
				}
	...
	case BoundVar:
		...
		s := lit[n+1:]
		if IsSegment(s) {
			return &BoundSegmentLiteral{Value: lit, BoundVar: BareBoundVariableLiteral{Value: lit[1:n]}, Segment: Segment(s)}, nil
		}
		...
		if IsField(s) {
			...
			return &BoundFieldLiteral{Value: lit, BoundVar: BareBoundVariableLiteral{Value: lit[1:n]}, Field: field}, nil
		}
...
func (p *Parser) parseField(name string) (*FieldLiteral, error) {
	...
	if tok == LBracket {
		...
		return &FieldLiteral{Value: name, Field: Field(name), Arg: lit}, nil
	}
	// unscan lbracket
	p.unscan()
	...
	return &FieldLiteral{Value: name, Field: Field(name)}, nil
}
